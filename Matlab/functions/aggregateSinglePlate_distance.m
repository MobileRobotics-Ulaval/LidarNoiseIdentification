function allData = aggregateSinglePlate_distance(data, sensor, plate, location, x)
...
for i  = 1:length(distance)
    if isfield(data.(sensor), location) ...
               && isfield(data.(sensor).(location).(plate), distance{i})
        allData = [allData ; data.(sensor).(location).(plate).(distance{i}).result.(x)];
    end
end